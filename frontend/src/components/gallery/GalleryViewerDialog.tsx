...
export function GalleryViewerDialog({
  ...
  const [mobileSection, setMobileSection] = useState<'details' | 'comments'>('details')
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm" onClick={onClose}>
      <div className="mx-auto flex min-h-screen w-full max-w-7xl items-center justify-center px-4 py-6">
        <div
          className="grid max-h-[92dvh] w-full overflow-hidden rounded-3xl border border-white/10 bg-card shadow-2xl lg:max-h-[90vh] lg:grid-cols-[1.35fr,0.65fr]"
          onClick={(event) => event.stopPropagation()}
        >
          <div
            className="relative flex max-h-[54dvh] min-h-[320px] min-w-0 items-center justify-center bg-black/70 p-4 sm:p-5 lg:max-h-none"
          ...
          >
            ...
            <div className="absolute left-1/2 top-4 z-10 -translate-x-1/2 rounded-full bg-black/45 px-3 py-1 text-[11px] font-medium text-white/80 backdrop-blur sm:hidden">
              위/아래 스와이프 또는 버튼으로 이동
            </div>
            <div className="absolute inset-x-4 bottom-4 z-10 flex gap-2 sm:hidden">
              <button
                onClick={onPrev}
                disabled={!canGoPrev}
                className="flex-1 rounded-2xl bg-black/60 px-4 py-3.5 text-sm font-medium text-white/95 backdrop-blur transition hover:bg-black/75 disabled:cursor-not-allowed disabled:opacity-35"
              >
                이전
              </button>
              <button
          ...
          <div className="flex min-h-0 flex-col p-4 sm:p-6">
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              <div className={mobileSection === 'details' ? 'block' : 'hidden sm:block'}>
                {detailsContent}
              </div>
              <div className={mobileSection === 'comments' ? 'mt-0 block' : 'mt-6 hidden sm:block'}>
                {commentsContent}
              </div>
            </div>
